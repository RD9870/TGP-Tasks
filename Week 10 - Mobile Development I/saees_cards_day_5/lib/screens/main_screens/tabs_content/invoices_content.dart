...
class _InvoicesContentState extends State<InvoicesContent> {
  @override
  void initState() {
    Provider.of<InvoicesProvider>(context, listen: false).setLisnter();
  ...
  Widget build(BuildContext context) {
    return Consumer<InvoicesProvider>(
      builder: (context, invoicesConsumer, _) {
        ...
        return ListView.builder(
          controller: invoicesConsumer.controller,
          itemCount: invoicesConsumer.invoices.length + 1,
          itemBuilder: (context, index) {
            if (index < invoicesConsumer.invoices.length) {
              return InvoiceCard(
                invoiceModel: invoicesConsumer.invoices[index],
              );
            } else {
              return invoicesConsumer.busy
                  ? const Padding(
